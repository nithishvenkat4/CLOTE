...
PREVIEWABLE_EXTS = {
    ...
    ".pdf",
    ".txt", ".md", ".csv", ".html", ".css", ".js", ".ts",
    ".py", ".java", ".cpp", ".c", ".go", ".rs", ".sh",
    ".json", ".yaml", ".yml", ".xml", ".toml", ".ini", ".env", ".log",
}

# MIME to use when stored mime is missing/wrong but extension is known
EXT_TO_MIME = {
...
def preview_file(file_id: int, user=Depends(get_current_user)):
    ...
    Accepts any image, PDF, or text/code file — by MIME or extension.
    """
    conn = get_db()
    try:
        row = conn.execute("SELECT * FROM files WHERE id = ?", (file_id,)).fetchone()
        ...
        stored_mime = row["mime_type"] or ""
        ext = Path(row["filename"]).suffix.lower()

        # Decide actual mime to serve
        if stored_mime in PREVIEWABLE_MIMES:
            serve_mime = stored_mime
        elif ext in PREVIEWABLE_EXTS:
            # Use known mapping or fall back to text/plain for code files
            serve_mime = EXT_TO_MIME.get(ext, "text/plain")
        else:
            raise HTTPException(
                status_code=415,
                detail=f"Preview not supported for this file type ({stored_mime or ext or 'unknown'})"
            )
